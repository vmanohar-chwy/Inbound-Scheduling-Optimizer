...
try:
    ...
    exp_sku = {}
    slot_count = {}
    logger = logging.getLogger('WFC_run')
    ...
    #giving slots 
    day_slots= ['05:30:00','06:00:00','06:30:00','07:30:00','08:00:00','08:30:00','09:30:00','10:00:00','10:30:00','11:30:00','12:00:00']
    night_slots=['15:00:00','15:30:00','16:00:00','16:30:00','17:00:00','17:30:00','18:00:00','18:30:00','19:00:00','19:30:00','20:00:00']
    ...
    std_no = {
    ('P000356','17:30:00'): '565',
    ('00000004','18:30:00'): '652',
    ('9000','16:00:00'): '656',
    ('00000007','16:00:00'): '657',
    ('2175','11:30:00'): '661',
    ('2189','11:30:00'): '707',
    ('2179','11:30:00'): '720',
    ('00000007','16:30:00'): '731',
    ('00000002','09:30:00'): '756',
    ('000000012','06:30:00'): '758',
    ('B000045','06:30:00'): '762'}
    ...
    WHERE apl.wh_id = 'WFC2' AND LOWER(apl.status) <> 'cancelled' AND apl.request_date:: DATE > current_date
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    AND fulfillment_center_name = 'WFC2'
    ...
    WHERE common_date_dttm between current_date-7 and current_date+123 AND location = 'WFC2'
    ORDER BY 1
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    WHERE apl.wh_id = 'WFC2' AND apl.status <> 'Cancelled' AND p.product_merch_classification2 = 'Litter'  AND p.product_vas_profile_description IN ('SHRINKWRAP') AND apl.request_date:: DATE >= current_date 
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    WITH data2 AS 
                ...
                WHERE cpl.FC_nm = 'WFC2'  AND cpl.Created_dt BETWEEN  (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'WFC2') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND')   
    ...
    query = """
            ...
            WHERE cpl.Created_dt BETWEEN (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'WFC2') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND') AND cpl.FC_nm LIKE 'WFC%' AND UPPER(cpl.request_type) LIKE 'CREATE%' AND cpl.VRDD:: DATE >= '20190701'
            GROUP BY 1,2
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    JOIN chewybi.vendors AS v
    ON pdm.vendor_key = v.vendor_key
    WHERE apl.wh_id = 'WFC2' AND LOWER(apl.status) <> 'cancelled' AND apl.request_date:: DATE >= '2019-07-01' AND v.vendor_number IN ('P000533','B000050','1760','9295','9302','P000544','P000508','P000486','P000400','7701','P000398','B000064','P000421','P000476','3755','3722','8038','5223')
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    if df.empty == False:    
        df.columns = ['date','cnt']
        cont_appt = dict([str(i),3-int(j)] for i,j in zip(df.date,df.cnt))
    else:
        cont_appt = {}
    ...
    WITH data AS (SELECT cpl.Ref_no, cpl.VRDD:: DATE, cpl.Created_dt:: DATE,pdm.document_number
            ...
            WHERE cpl.FC_nm LIKE 'WFC%' AND cpl.Ref_no NOT IN (SELECT Ref_no FROM  sandbox_supply_chain.iso_exception) AND cpl.Created_dt BETWEEN (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'WFC2') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND')  AND cpl.FC_nm LIKE 'WFC%' AND cpl.Ref_no NOT IN (SELECT Ref_no FROM  sandbox_supply_chain.iso_exception)
    )
    ...
    USING(appointment_id)
    WHERE apl.request_date:: DATE BETWEEN current_date+1 AND current_date+35 AND LOWER(apl.status) <> 'cancelled' AND apl.standing_appt_id IS NOT NULL AND apl.vendor IS NOT NULL AND apl.wh_id = 'WFC2' AND pol.po_number IS NULL
    ORDER BY 1,2
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    where vendor not in ('AVP1','CFC1','DAY1','DFW1','EFC3','MCO1','PHX1','WFC2') and ss.wh_id = 'WFC2'
    group by 1,2,3,4,5,6
    order by 1,4,2
    )
    ...
    WITH break AS 
           ...
           JOIN aad.t_po_detail AS pdp
           ON pol.po_number = pdp.po_number
           WHERE apl.status <> 'Cancelled' AND apl.request_date:: DATE BETWEEN current_date AND current_date+60 AND apl.wh_id = 'WFC2'
           GROUP BY 1,2,3
           ORDER BY 1,2
           )
    ,break_final AS 
    ...
    for (i,j) in sch_sh.keys():
        if j == '1':
            slot_count[(i,j)] = len(day_slots)*2  
        else:
            slot_count[(i,j)] = len(night_slots)*2 
    ...
    for j in cont_appt.keys():
        if cont_appt[j] == 0:
            ...
            sch_slot[(j,'09:30:00','c')] = 1
            sch_slot[(j,'14:30:00','c')] = 1
        elif cont_appt[j] == 2:
            sch_slot[(j,'05:00:00','c')] = 1
            sch_slot[(j,'09:30:00','c')] = 1
            sch_slot[(j,'14:30:00','c')] = 0
        elif cont_appt[j] == 1:
            sch_slot[(j,'05:00:00','c')] = 1
            sch_slot[(j,'09:30:00','c')] = 0
            sch_slot[(j,'14:30:00','c')] = 0
        else:
                    ...
                    for l in stnd_time[vendor[j]]:
                        if (k,l,vendor[j]) in stnd_fl.keys():
                            ...
                            if stnd_fl[(k,l,vendor[j])] == '0' and p == 0 and slot[k] < (slot_count[(k,'1')] + slot_count[(k,'2')]) and units[k] < 1.05 * f[k]:
                                if l in day_slots and sch_sh[(k,'1')][2] < 22: 
                                    if (k,l) in out_3:
                                        out_3[(k,l)].append(j)
                                    else:
                                        out_3[(k,l)] = [j]
                                ...
                                elif l in night_slots and sch_sh[(k,'2')][2] < 22:
                                    if (k,l) in out_3:
                                        out_3[(k,l)].append(j)
                                    else:
                                        out_3[(k,l)] = [j]
    ...
    while True:
        ...
        if m1.status == GRB.OPTIMAL or m1.status == GRB.TIME_LIMIT:
            ...
            m1.write('day_model_WFC.ilp')#writing causes of infeasibility
    ...
    if m1.status == GRB.OPTIMAL or m1.status == GRB.TIME_LIMIT:        
        for j in infeas_ref:
            for k in ref_num[j]:
                infeasible_day[(j,k)] = [str(TODAY),'WFC2','None',str(j),str(k),dt[j][0],cr_dt[j],'NOT_OPTIMAL']
    else:
        for j in dt.keys():
            for k in ref_num[j]:
                infeasible_day[(j,k)] = [str(TODAY),'WFC2','None',str(j),str(k),dt[j][0],cr_dt[j],'NOT_OPTIMAL']
    ...
    for j in out_1.keys():
        ...
        if m2.status == GRB.OPTIMAL or m2.status == GRB.TIME_LIMIT:
            ...
            m2.write('shift_model_WFC.ilp')#writing causes of infeasibility
            ...
            for k in std:
                for l in ref_num[k]:
                    infeas_shift[(k,l)] = [str(TODAY),'WFC2','None',str(k),str(l),dt[k][0],cr_dt[k],'NOT_OPTIMAL']
                    ...
                    if sch_slot[(k,'05:00:00','c')] < 1:
                        if p == 0:
                            if (k,'05:00:00') in out_3:
                                ...
                                bulk_break[(k,'05:00:00')] = '1'
                                bulk_break[(k,'05:30:00')] = '2'
                    elif sch_slot[(k,'09:30:00','c')] < 1:
                        if p == 0:
                            if (k,'09:30:00') in out_3:
                                out_3[(k,'09:30:00')].append(a)
                                p = 1
                                sch_slot[(k,'09:30:00','c')] = sch_slot[(k,'09:30:00','c')] + 1
                                bulk_break[(k,'09:30:00')] = '1'
                                bulk_break[(k,'09:00:00')] = '2'
                                bulk_break[(k,'10:00:00')] = '2'
                            else:
                                out_3[k,'09:30:00'] = [a]
                                p = 1
                                sch_slot[(k,'09:30:00','c')] = sch_slot[(k,'09:30:00','c')] + 1
                                ...
                                bulk_break[(k,'09:00:00')] = '2'
                                bulk_break[(k,'10:00:00')] = '2'
                    elif sch_slot[(k,'14:30:00','c')] < 1:
                        if p == 0:
                            if (k,'14:30:00') in out_3:
                                out_3[(k,'14:30:00')].append(a)
                                p = 1
                                sch_slot[(k,'14:30:00','c')] = sch_slot[(k,'14:30:00','c')] + 1
                                bulk_break[(k,'14:30:00')] = '1'
                                bulk_break[(k,'14:00:00')] = '2'
                            else:
                                out_3[k,'14:30:00'] = [a]
                                p = 1
                                sch_slot[(k,'14:30:00','c')] = sch_slot[(k,'14:30:00','c')] + 1
    ...
    out_file = open('SCHEDULE_WFC_UNITS.csv','w')
    ...
    out_file = open('SCHEDULE_WFC_'+str(date)+'_.csv','w')
        ...
        for k in out_3[(i,j)]:
            for l in ref_num[k]:
                if (vendor[k],j) in std_no:
                    ...
                    out_file.write('\n')
                    data[M3,l] = [a,k,l,i,j,po[(k,l)][0],po[(k,l)][1],v_name[k],cr_dt[k],'WFC2']
                else:
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Algorithm Successfully ran for WFC2" 
    ...
    if M1 > 0 or M2 > 0:
        if M1 > 0 and M2==0:
            ...
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for WFC2 at day level" 
            ...
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for WFC2 at shift level" 
            ...
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for WFC2 at day level and  shift level" 
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Algorithm did not run for WFC2" 
