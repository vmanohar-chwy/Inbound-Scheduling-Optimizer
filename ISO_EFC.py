# -*- coding: utf-8 -*-
"""
Created on Tue Sep  3 09:21:38 2019
...
try:
    ...
    logger = logging.getLogger('EFC_run')
    ...
    slot_count = {}
    #giving slots 
    day_slots= ['06:00:00','06:30:00','07:30:00','08:00:00','08:30:00','09:30:00','10:00:00','10:30:00','11:30:00','12:30:00','13:00:00']
    night_slots=['16:00:00','16:30:00','17:00:00','17:30:00','18:00:00','18:30:00','19:00:00','19:30:00','20:30:00','21:00:00']
    ...
    std_no= {
    ('B000045','08:00:00'): '259',
    ('B000046','17:00:00'): '263',
    ('B000046','21:00:00'):'265',
    ('B000032','13:00:00'): '391',
    ('9000','12:30:00'): '530',
    ('B000122','08:00:00'): '535',
    ('B000045','16:00:00'): '538',
    ('P000356','17:00:00'): '553',
    ('P000356','20:30:00'): '554',
    ('9011','20:30:00'): '603',
    ('1458','20:30:00'): '628',
    ('00000002','15:30:00'): '699',
    ('B000047','10:00:00'): '700',
    ('1195','06:00:00'): '703',
    ('1277','07:00:00'): '704',
    ('000000012','09:30:00'): '705',
    ('00000007','11:00:00'): '763',
    ('00000004','08:30:00'): '765'}
    ...
    WHERE apl.wh_id = 'EFC3' AND LOWER(apl.status) <> 'cancelled' AND apl.request_date:: DATE >= current_date
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    AND forecast_week_beg BETWEEN DATE_TRUNC('week',current_date-30):: DATE -1 AND DATE_TRUNC('week',current_date+112):: DATE-1
    AND fulfillment_center_name = 'EFC3'
    ...
    WHERE common_date_dttm between current_date-7 and current_date+123 AND location = 'EFC3'
    ORDER BY 1
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    WHERE apl.wh_id = 'EFC3' AND apl.status <> 'Cancelled' AND p.product_merch_classification2 = 'Litter'  AND p.product_vas_profile_description IN ('SHRINKWRAP') AND apl.request_date:: DATE >= current_date 
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    WITH data2 AS 
                ...
                WHERE cpl.FC_nm = 'EFC3'  AND cpl.Created_dt BETWEEN  (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'EFC3') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND')   
        ...
        ,cont_flag AS
                ...
                END AS cont_flag
                FROM data AS d
                JOIN aad.t_po_master AS pod
                ON d.PO_no = pod.po_number
                JOIN chewybi.vendors AS v
                ON pod.vendor_code = v.vendor_number
                )
        ,cont_fl AS
                (
    ...
    query = """
            ...
            WHERE cpl.Created_dt BETWEEN (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'EFC3') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND') AND cpl.FC_nm LIKE 'EFC%' AND UPPER(cpl.request_type) LIKE 'CREATE%' AND cpl.VRDD:: DATE >= '20190701'
            GROUP BY 1,2
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    WHERE apl.wh_id = 'EFC3' AND LOWER(apl.status) <> 'cancelled' AND apl.request_date:: DATE >= current_date AND v.vendor_number IN ('P000533','B000050','1760','9295','9302','P000544','P000508','P000486','P000400','7701','P000398','B000064','P000421','P000476','3755','3722','8038','5223')
    GROUP BY 1
    ORDER BY 1
    """
    #cur.execute(query)
    ...
    WITH data AS (SELECT cpl.Ref_no, cpl.VRDD:: DATE, cpl.Created_dt:: DATE,pdm.document_number
            ...
            WHERE cpl.FC_nm LIKE 'EFC%' AND cpl.Ref_no NOT IN (SELECT Ref_no FROM  sandbox_supply_chain.iso_exception) AND cpl.Created_dt BETWEEN (SELECT MAX(Created_dt) FROM sandbox_supply_chain.ISO_OUTPUT_NEW WHERE FC_nm = 'EFC3') + INTERVAL '1 SECOND' AND (SELECT current_date - INTERVAL '1 SECOND')  
    )
    ...
    USING(appointment_id)
    WHERE apl.request_date:: DATE BETWEEN current_date+1 AND current_date+35 AND LOWER(apl.status) <> 'cancelled' AND apl.standing_appt_id IS NOT NULL AND apl.vendor IS NOT NULL AND apl.wh_id = 'EFC3' AND pol.po_number IS NULL
    ORDER BY 1,2
    """
    #cur.execute(query)
    #result = cur.fetchall()
    ...
    where vendor not in ('AVP1','CFC1','DAY1','DFW1','EFC3','MCO1','PHX1','WFC2') and ss.wh_id = 'EFC3'
    group by 1,2,3,4,5,6
    order by 1,4,2
    )
    ...
    WITH break AS 
           ...
           JOIN aad.t_po_detail AS pdp
           ON pol.po_number = pdp.po_number
           WHERE apl.status <> 'Cancelled' AND apl.request_date:: DATE BETWEEN current_date AND current_date+60 AND apl.wh_id = 'EFC3'
           GROUP BY 1,2,3
           ORDER BY 1,2
           )
    ,break_final AS 
    ...
    for (i,j) in sch_sh.keys():
        if j == '1':
            slot_count[(i,j)] = len(day_slots)*2 + 3  
        else:
            slot_count[(i,j)] = len(night_slots)*2 + 3
    
    
    logger.info("Initialized basic data structures")
    ...
    for j in cont_appt.keys():
        if cont_appt[j] == 0:
            sch_slot[(j,'06:00:00','c')] = 1
            sch_slot[(j,'16:00:00','c')] = 1
        elif cont_appt[j] == 1:
            sch_slot[(j,'06:00:00','c')] = 1
            sch_slot[(j,'16:00:00','c')] = 0
            if (j,'16:00:00') in sch_slot:
                sch_slot[(j,'16:00:00')] = sch_slot[(j,'16:00:00')] + 1
            else:
                 sch_slot[(j,'16:00:00')] = 1
            sch_sh[(j,'2')][2] = sch_sh[(j,'2')][2] + 1
        elif cont_appt[j] == 2:
            sch_slot[(j,'06:00:00','c')] = 0
            sch_slot[(j,'16:00:00','c')] = 0
            if (j,'06:00:00') in sch_slot:
                sch_slot[(j,'06:00:00')] = sch_slot[(j,'06:00:00')] + 1
            else:
                 sch_slot[(j,'06:00:00')] = 1
            if (j,'16:00:00') in sch_slot:
                sch_slot[(j,'16:00:00')] = sch_slot[(j,'16:00:00')] + 1
            else:
                 sch_slot[(j,'16:00:00')] = 1
            sch_sh[(j,'2')][2] = sch_sh[(j,'2')][2] + 1
            sch_sh[(j,'1')][2] = sch_sh[(j,'1')][2] + 1
        else:
            pass
    ...
    for j in sch_dict.keys():
        ...
        slot[j] = sch_sh[(j,'1')][2] + sch_sh[(j,'2')][2]        
                ...
                if k in stnd_date[vendor[j]]:
                    ...
                    for l in stnd_time[vendor[j]]:
                        if (k,l,vendor[j]) in stnd_fl.keys():
                            cnt = cnt+1
                            if stnd_fl[(k,l,vendor[j])] == '0' and p == 0 and slot[k]+1 < (slot_count[(k,'1')] + slot_count[(k,'2')]) and units[k]+units_sku_obj[j][0] < 1.05 * f[k]:
                                if (l in day_slots or l in ['14:00:00','05:30:00']) and sch_sh[(k,'1')][2] < slot_count[(k,'1')]: 
                                    if (k,l) in out_3:
                                        out_3[(k,l)].append(j)
                                    else:
                                        out_3[(k,l)] = [j]
                                ...
                                elif (l in night_slots or l in ['15:30:00']) and sch_sh[(k,'2')][2] < slot_count[(k,'2')]:
                                    if (k,l) in out_3:
                                        out_3[(k,l)].append(j)
                                    else:
                                        out_3[(k,l)] = [j]
                                ...
                                if p == 1:
                                    ...
                                    if k in std_ref:
                                        ...
                                        std_ref[k] = [j]
                                    if (l in day_slots or l in ['14:00:00','05:30:00']):
                                        if (k,1) in std_sh:
                                            std_sh[(k,1)].append(j)
                                        else:
                                            std_sh[(k,1)] = [j]
                                ...
                                if p == 0 and slot[k] + 1 < (slot_count[(k,'1')] + slot_count[(k,'2')]) - 5 and units[k] + exp_units[(k,'1')] + exp_units[(k,'2')] + units_sku_obj[j][0]  < 1.05 * f[k] and cnt > len(stnd_time[vendor[j]]):
                                    if k in std_ref:
                                        std_ref[k].append(j)
                                    else:
                                        std_ref[k] = [j]
    ...
    for i in sch_dict.keys():
        if (sch_dict[i][0] < 1.05 * f[i]) and (sch_sh[(i,'1')][2] >= len(day_slots)*2 and sch_sh[(i,'2')][2] >= len(night_slots)*2):
    ...
    while True:
        ...
        #unit distribution constraint
            ...
            day_assign[j] = m1.addConstr(quicksum(x[k,j] for k in ref[j] if k not in infeas_ref and k not in stnd_ref2), GRB.LESS_EQUAL,(slot_count[(j,'1')]+slot_count[(j,'2')]) - (sch_sh[(j,'1')][2]+sch_sh[(j,'2')][2]), name='day_assign[%s]' %(j))
        ...
        for j in dt.keys():
            ...
            m1.update()
        for j in dt.keys():
            if j not in infeas_ref and j not in stnd_ref2 and vas_flag[j] == '1' and j not in stnd_ref:
                vas_appt_assign[j] = m1.addConstr(quicksum(x[j,k] for k in dt[j] if k not in vas_dt),GRB.GREATER_EQUAL,1)
                
            m1.update()
        #standing_appointment_constraint
        for j in std_ref.keys():
            for k in std_ref[j]:
                if k not in infeas_ref and j not in stnd_ref2:
        ...
        m1.write('day_model_EFC.lp') #writing the day model
        ...
        if m1.status == GRB.OPTIMAL or m1.status == GRB.TIME_LIMIT:
            ...
            m1.write('day_model_EFC.ilp')#writing causes of infeasibility
    ...
    if m1.status == GRB.OPTIMAL or m1.status == GRB.TIME_LIMIT:        
        for j in infeas_ref:
            for k in ref_num[j]:
                infeasible_day[(j,k)] = [str(TODAY),'EFC3','None',str(j),str(k),dt[j][0],cr_dt[j],'NOT_OPTIMAL']
    else:
        for j in dt.keys():
            for k in ref_num[j]:
                infeasible_day[(j,k)] = [str(TODAY),'EFC3','None',str(j),str(k),dt[j][0],cr_dt[j],'NOT_OPTIMAL']
    ...
    for j in out_1.keys():
        ...
        m2.update()
        for k in out_1[j]:
            if vas_flag[k] == '1' and k not in stnd_ref:
                vas_shift_appt[j,k]= m2.addConstr(quicksum(y[j,k,l] for l in range(1,3) if (j,str(l)) not in vas_dt),GRB.GREATER_EQUAL,0)
        ...
        unit_shift[j,1]= m2.addConstr(US,GRB.GREATER_EQUAL,f_sh[(j,1)]-sch_sh[(j,'1')][0]-day1[(j,1)]-exp_units[(j,'1')],name='unit_shift[%s;%d]'%(j,1))
        unit_shift[j,3]= m2.addConstr(US,GRB.GREATER_EQUAL,sch_sh[(j,'1')][0]+day1[(j,1)]+exp_units[(j,'1')]-f_sh[(j,1)],name='unit_shift[%s;%d]'%(j,3))
        #night shift unit limitation
        unit_shift[j,2]= m2.addConstr(US,GRB.GREATER_EQUAL,f_sh[(j,2)]-sch_sh[(j,'2')][0]-night1[(j,1)]-exp_units[(j,'2')],name='unit_shift[%s;%d]'%(j,2))
        unit_shift[j,4]= m2.addConstr(US,GRB.GREATER_EQUAL,sch_sh[(j,'2')][0]+night1[(j,1)]+exp_units[(j,'2')]-f_sh[(j,2)],name='unit_shift[%s;%d]'%(j,4))
        ...
        m2.write('shift_model_EFC.lp')#writing shift model
        ...
        if m2.status == GRB.OPTIMAL or m2.status == GRB.TIME_LIMIT:
            ...
            m2.write('shift_model_EFC.ilp')#writing causes of infeasibility
            ...
            for k in std:
                for l in ref_num[k]:
                    infeas_shift[(k,l)] = [str(TODAY),'EFC3','None',str(k),str(l),dt[k][0],cr_dt[k],'NOT_OPTIMAL']
        ...
        while len(out_2[(k,j)]) > 0:
            ...
            if a not in stnd_ref:
                ...
                if cnt_fl[a] == '3.0': #container appointments
                    if sch_slot[(k,'06:00:00','c')] < 1:
                        if p == 0:
                            if (k,'06:00:00') in out_3:
                                out_3[(k,'06:00:00')].append(a)
                                p = 1
                                sch_slot[(k,'06:00:00','c')] = sch_slot[(k,'06:00:00','c')] + 1
                                bulk_break[(k,'06:00:00')] = '1'
                                bulk_break[(k,'06:30:00')] = '2'
                            else:
                                out_3[k,'06:00:00'] = [a]
                                p = 1
                                sch_slot[(k,'06:00:00','c')] = sch_slot[(k,'06:00:00','c')] + 1
                                bulk_break[(k,'06;00:00')] = '1'
                                bulk_break[(k,'06:30:00')] = '2'
                    elif sch_slot[(k,'16:00:00','c')] < 1:
                        if p == 0:
                            if (k,'16:00:00') in out_3:
                                out_3[(k,'16:00:00')].append(a)
                                p = 1
                                sch_slot[(k,'16:00:00','c')] = sch_slot[(k,'16:00:00','c')] + 1
                                bulk_break[(k,'16:00:00')] = '1'
                                bulk_break[(k,'16:30:00')] = '2'
                            else:
                                out_3[k,'16:00:00'] = [a]
                                p = 1
                                sch_slot[(k,'16:00:00','c')] = sch_slot[(k,'16:00:00','c')] + 1
                    ...
                    if p == 1:
                       ...
                       for d in sorted(day_slots):
                           if sch_vas_fl[(k,d)] == '1' and gh != 0 and gh != len(day_slots)-1:
                               ...
                               w = gh+1
                               sch_vas_fl[(k,day_slots[w])] = '2'
                           elif sch_vas_fl[(k,d)] == '1' and gh == len(day_slots)-1:
                               w = gh-1
                               sch_vas_fl[(k,day_slots[w])] = '2'
                           else:
                               pass
                        ...
                        if p == 1:
                           ...
                           for d in sorted(day_slots):
                               if bulk_break[(k,d)] == '1' and gh != 0 and gh != len(day_slots)-1:
                                   ...
                                   if w < len(day_slots):
                                       bulk_break[(k,day_slots[w])] = '2'
                               elif bulk_break[(k,d)] == '1' and gh == 0:
                                   w = gh+1
                                   bulk_break[(k,day_slots[w])] = '2'
                               elif bulk_break[(k,d)] == '1' and gh == len(day_slots)-1:
                                   w = gh-1
                                   bulk_break[(k,day_slots[w])] = '2'
                               else:
                                   pass
    ...
    logger.info("Completed Scheudling time slots")     
    print(M1)
    ...
    out_file = open('SCHEDULE_EFC_UNITS.csv','w')
    ...
    out_file = open('SCHEDULE_EFC_'+str(date)+'_.csv','w')
        ...
        for k in out_3[(i,j)]:
            for l in ref_num[k]:
                if (vendor[k],j) in std_no:
                    ...
                    out_file.write('\n')
                    data[M3,l] = [a,k,l,i,j,po[(k,l)][0],po[(k,l)][1],v_name[k],cr_dt[k],'EFC3']
                else:
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Algorithm Successfully ran for EFC3" 
    ...
    if M1 > 0 or M2 > 0:
        if M1 > 0 and M2==0:
            ...
            to = ', '.join(toaddr)
            file_list = ['day_model_EFC.ilp']
            msg = MIMEMultipart()
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for EFC3 at day level"+str(M1)+"times" 
            ...
            toaddr = 'vmanohar@chewy.com,igonzalez1@chewy.com,EAlfonso@chewy.com,jxie@chewy.com'
            to = ', '.join(toaddr)
            file_list = ['shift_model_EFC.ilp']
            msg = MIMEMultipart()
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for EFC3 at shift level"+str(M2)+"times" 
            ...
            toaddr = 'vmanohar@chewy.com,igonzalez1@chewy.com,EAlfonso@chewy.com,jxie@chewy.com'
            to = ', '.join(toaddr)
            file_list = ['shift_model_EFC.ilp','day_model_EFC.ilp']
            msg = MIMEMultipart()
            msg['From'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "LP Model Failed for EFC3 at day level and  shift level" 
            body = "Hello, \nModel Failed at day level"+str(M1)+"times and shift level"+str(M1)+"times.\nThanks\nVenkatesh"
            msg.attach(MIMEText(body, 'plain'))
            for j in file_list:
                file_path = j
                attachment = open(file_path, "rb")
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Algorithm did not run for EFC3" 
