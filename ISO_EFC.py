...
try:
    ...
    where iblm.date::date >= current_date AND iblm.wh_id = 'EFC3'
    order by wh_id, date;
    """
    df = pd.read_sql(query,cxn)
...
# =============================================================================
                ...
                if k in stnd_date[vendor[j]]:
                    ...
                    for l in stnd_time[vendor[j]]:
                        if (k,l,vendor[j]) in stnd_fl.keys():
                            cnt = cnt+1
                            if stnd_fl[(k,l,vendor[j])] == '0' and p == 0 and slot[k]+1 < (slot_count[(k,'1')] + slot_count[(k,'2')]) and units[k]+units_sku_obj[j][0] < 1.05 * (f[k][0]+f[k][1]):
    ...
    for i in sch_dict.keys():
        if (sch_dict[i][0] < 1.05 * (f[i][0]+f[i][1])) and (sch_sh[(i,'1')][2] >= (len(day_slots)*2) -4 and sch_sh[(i,'2')][2] >= (len(night_slots)*2)-4):
        ...
        for k in out_3[(i,j)]:
            for l in ref_num[k]:
                if (vendor[k],j) in std_no:
                    ...
                    out_file.write('\n')
                    data[M3,l] = [a,k,l,i,j,po[(k,l)][0],po[(k,l)][1],v_name[k],cr_dt[k],'EFC3','Daily']
                else:
    ...
    for i,j in sorted(out_3.keys()):
        ...
        for k in out_3[(i,j)]:
            for l in ref_num[k]:
                bulk_e[cnt] = [str(k),str(l),str(i),str(j),a,'0',a,'1',a,'1','EFC3',int(inc[k]),'419',str(loc)]
                cnt = cnt+1
